...
export default function Dashboard() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
          ...
          <CardContent className="pt-5 pb-5">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-2xl font-bold mt-0.5">{studiedCount}<span className="text-sm text-muted-foreground font-normal">/{UNITS.length}</span></p>
            ...
            <Progress value={(studiedCount / UNITS.length) * 100} className="mt-3 h-1" />
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-5 pb-5">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-2xl font-bold mt-0.5">{completedExercises}<span className="text-sm text-muted-foreground font-normal">/{totalExercises}</span></p>
                <p className="text-xs text-muted-foreground mt-0.5">Interactive practice</p>
              </div>
              <FlaskConical className="w-5 h-5 text-violet-400 mt-0.5" />
            </div>
            <Progress value={(completedExercises / totalExercises) * 100} className="mt-3 h-1" />
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-5 pb-5">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-2xl font-bold mt-0.5">{streak}<span className="text-sm text-muted-foreground font-normal"> days</span></p>
                <p className="text-xs text-muted-foreground mt-0.5">{examAttempted} exam Qs done</p>
              </div>
              <RotateCcw className="w-5 h-5 text-emerald-400 mt-0.5" />
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      {/* Units grid */}
      <h2 className="text-lg font-semibold mb-4">Learning Units</h2>
        ...
        {UNITS.map((unit, i) => {
          ...
          return (
            <Card
              ...
              className={`border ${unitColors[i]} transition-all hover:shadow-md`}
            >
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <Badge variant="outline" className={`text-xs px-2 py-0 ${unitBadgeColors[i]}`}>
                        LU{i + 1}
                      </Badge>
                      {isStudied && (
                        <CheckCircle2 className={`w-4 h-4 ${unitAccents[i]}`} />
                      )}
                    </div>
                    <CardTitle className="text-base font-semibold leading-snug">{unit.shortTitle}</CardTitle>
              ...
              <CardContent className="pt-0">
                ...
                {exercisePct > 0 && (
                  <Progress value={exercisePct} className="h-1 mb-3" />
                )}

                <div className="flex gap-2">
                  <Link href={`/learn/${unit.id}`}>
                    <Button
                      data-testid={`btn-learn-${unit.id}`}
                  ...
                  <Link href={`/practice/${unit.id}`}>
                    <Button
                      data-testid={`btn-practice-${unit.id}`}
                      variant="outline"
                      size="sm"
                      className="gap-1.5 h-8 text-xs"
                    >
      ...
      <Card className="mt-6 border-primary/20 bg-primary/5">
        <CardContent className="pt-5 pb-5">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-semibold text-sm">Ready for the real thing?</h3>
